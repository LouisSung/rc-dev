...
# docker run -idt --name rc-dev -v rc-db:/root/Rocket.Chat/.meteor/local/db -p 2222:22 -p 3000:3000 -p 3001:3001 -p 9229:9229 louissung/rc:dev-3.16.1
...
# docker run -idt --name rc-prod --network=rc-network -v rc-uploads:/root/app/uploads -v "$(pwd)/docker/docker-entrypoint.sh":/usr/local/bin/docker-entrypoint.sh -p 3000:3000 louissung/rc:dev-3.16.1

# ===== deploy mode =====
# same setup as prod mode (network, volume, and mongodb)
# docker run -idt --name rc-deploy --network=rc-network -v rc-uploads:/app/uploads  -v "$(pwd)/app/bundle:/app/bundle" -p 3000:3000 -e MONGO_URL=mongodb://rc-mongo:27017/rocketchat louissung/rc:runtime-3.16.1
